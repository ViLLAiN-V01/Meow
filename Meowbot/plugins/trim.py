...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            mewevent = await eor(event, "`Saving the file...`")
            try:
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, mewevent, c_time, "trying to download")
                    ),
                )
...
async def ff_mpeg_trim_cmd(event):
    ...
    mewevent = await eor(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await eod(
                mewevent, f"**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, mewevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await eod(mewevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        ...
        await eod(mewevent, "RTFM")
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await eod(mewevent, f"`Completed Process in {ms} seconds`", 3)


@bot.on(mew_cmd(pattern="atrim"))
