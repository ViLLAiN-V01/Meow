...
async def fsub_in(event):
        ...
        if event.query.user_id == bot.uid and query == "fsub":
            ...
            mew = hunter.split("+")
            user = await bot.get_entity(int(mew[0]))
            channel = await bot.get_entity(int(mew[1]))
...
async def on_pm_click(event):
    ...
    mew = hunter.split("+")
    if not event.sender_id == int(mew[0]):
        return await event.answer("This Ain't For You!!", alert=True)
    try:
        await bot(GetParticipantRequest(int(mew[1]), int(mew[0])))
    except UserNotParticipantError:
        return await event.answer(
            "You need to join the channel first.", alert=True
        )
    await bot.edit_permissions(
        event.chat_id, int(mew[0]), send_message=True, until_date=None
    )
