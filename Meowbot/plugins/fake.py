...
async def _(event):
    ...
    if response.status_code == 200:
      with open("MEOWBOT.jpg", 'wb') as f:
        f.write(response.content)
    
    captin = f"Fake Image By {mew_mention}"
    fole = "MEOWBOT.jpg"
    await bot.send_file(event.chat_id, fole, caption=captin)
    await event.delete()
    os.system("rm /root/Meowbot/MEOWBOT.jpg ")


@bot.on(mew_cmd(pattern="fake ?(.*)"))
