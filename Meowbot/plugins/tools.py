...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    mewevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(mewevent, "`Please unblock `@DrWebBot `and try again`")
            return
...
@bot.on(mew_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mewevent = await eor(event, "...")
...
@bot.on(mew_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    if len(input_sgra) == 3:
        ...
        await Meow.edit(str(a))
    else:
        await eod(Meow, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(mew_cmd(pattern="currency (.*)"))
...
@bot.on(mew_cmd(pattern="currencies$"))
...
@bot.on(mew_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mewevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
@bot.on(mew_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(mew_cmd(pattern="unshort (.*)", outgoing=True))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for MeowBot", ".currency 10 usd inr"
).add_command(
