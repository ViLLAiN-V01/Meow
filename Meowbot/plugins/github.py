...
@bot.on(mew_cmd(pattern=r"commit"))
...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        await eod(event, "`Please ADD Proper Github Repo Name of MeowBot`")
        return
    Meowbot = await eor(event, "Processing ...")
...
async def git_commit(file_name, Meowbot):
    content_list = []
