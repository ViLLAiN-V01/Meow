...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    Meow = await eor(event, "recognizeing this media")
    async with event.client.conversation(chat) as conv:
        ...
        if second.text.startswith("See next message."):
            ...
            Meow = third.message.message
            await eor(event, Meow)
...
@bot.on(mew_cmd(pattern="info ?(.*)", outgoing=True))
...
@bot.on(mew_cmd(pattern="chatinfo(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="chatinfo(?: |$)(.*)", allow_sudo=True))
async def info(event):
    if event.fwd_from:
        return
    Meow = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await Meow.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(Meow, "`An unexpected error has occurred.`")
    return
...
@bot.on(mew_cmd(pattern=r"users ?(.*)", outgoing=True))
...
@bot.on(mew_cmd(pattern="id$"))
@bot.on(sudo_cmd(pattern="id$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Meow = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await Meow.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
