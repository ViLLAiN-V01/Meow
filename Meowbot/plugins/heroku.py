...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        execl(executable, executable, "bash", "MeowBot")


@bot.on(mew_cmd(pattern="restart$"))
@bot.on(sudo_cmd(pattern="restart$", allow_sudo=True))
async def re(Meow):
    if Meow.fwd_from:
        return
    event = await eor(Meow, "Restarting Dynos ...")
    if HEROKU_API_KEY:
        await restart(event)
    else:
        await event.edit("Please Set Your `HEROKU_API_KEY` to restart ℳêøաɮøƚ")


@bot.on(mew_cmd(pattern="shutdown$"))
...
@bot.on(mew_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", outgoing=True))
@bot.on(sudo_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", allow_sudo=True))
async def variable(Meow):
    if Meow.fwd_from:
        return
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await eor(Meow, "`[HEROKU]:" "\nPlease setup your` **HEROKU_APP_NAME**")
    exe = Meow.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(Meow, "Getting Variable Info...")
        await asyncio.sleep(1.5)
        cap = "Logger me chala jaa bsdk."
        capn = "Saved in LOGGER_ID !!"
        try:
            variable = Meow.pattern_match.group(2).split()[0]
            if variable in ("MEOWBOT_SESSION", "BOT_TOKEN", "HEROKU_API_KEY"):
                if Config.ABUSE == "ON":
        ...
        event = await eor(Meow, "Setting Heroku Variable...")
        variable = Meow.pattern_match.group(2)
        if not variable:
            return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        value = Meow.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = Meow.pattern_match.group(2).split()[1]
            except IndexError:
                return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        await asyncio.sleep(1.5)
        if variable in heroku_var:
        ...
        event = await eor(Meow, "Getting info to delete Variable")
        try:
            variable = Meow.pattern_match.group(2).split()[0]
        except IndexError:
...
async def dyno_usage(Meow):
    if Meow.fwd_from:
        return
    event = await edit_or_reply(Meow, "`Processing...`")
    useragent = (
...
@bot.on(mew_cmd(pattern="logs$"))
@bot.on(sudo_cmd(pattern="logs$", allow_sudo=True))
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit {mew_grp} for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)
    except BaseException:
        return await dyno.reply(f"Make Sure Your Heroku AppName & API Key are filled correct. Visit {mew_grp} for help.", link_preview=False)
   # event = await eor(dyno, "Downloading Logs...")
    mew_data = app.get_log()
    await eor(dyno, mew_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :**  {mew_mention}\n\n🚀** Pasted**  ")
