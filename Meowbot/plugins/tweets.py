...
async def nope(mew):
    ...
    okvai = await eor(mew, "Trying to tweet for you...")
    if not Meow:
        if mew.is_reply:
            (await mew.get_reply_message()).message
        else:
            await eod(mew, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(Meow))}")
...
@bot.on(mew_cmd(pattern=r"trump(?: |$)(.*)"))
...
@bot.on(mew_cmd(pattern=r"pappu(?: |$)(.*)"))
...
@bot.on(mew_cmd(pattern=r"sunny(?: |$)(.*)"))
...
@bot.on(mew_cmd(pattern=r"johhny(?: |$)(.*)"))
...
@bot.on(mew_cmd(pattern=r"gandhi(?: |$)(.*)"))
...
@bot.on(mew_cmd(pattern=r"kanna(?: |$)(.*)"))
