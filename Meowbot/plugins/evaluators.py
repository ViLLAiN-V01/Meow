...
async def _(event):
    ...
    mewevent = await eor(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    mewuser = await event.client.get_me()
    if mewuser.username:
...
async def _(event):
    ...
    mewevent = await eor(event, "`Running ...`")
    ...
    if exc:
        ...
        evaluation = "Success"
    final_output = f"•  Eval : \n`{cmd}` \n\n•  Result : \n`{evaluation}` \n"
    await eor(mewevent, "**Eval Command Executed. Check out LOGGER for result.**")
    await event.client.send_message(
        lg_id,
...
@bot.on(mew_cmd(pattern="bash ?(.*)", outgoing=True))
...
CmdHelp("evaluators").add_command(
  ...
  "exec", "<command>", "Execute a Terminal command on MeowBot server and shows details"
).add_command(
