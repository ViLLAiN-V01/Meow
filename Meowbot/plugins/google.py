...
@bot.on(mew_cmd(pattern="wikipedia (.*)"))
...
@bot.on(mew_cmd(pattern="watch (.*)"))
@bot.on(sudo_cmd(pattern="watch (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    query = event.pattern_match.group(1)
    Meow = await eor(event, "Finding Sites...")
...
@bot.on(mew_cmd(pattern="img (.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="img (.*)", allow_sudo=True))
async def img(event):
    sim = event.pattern_match.group(1)
    if not sim:
        return await eod(event, "`Give something to search...`")
    Meow = await eor(event, f"Searching for  `{sim}`...")
    if "-" in sim:
        try:
            lim = int(sim.split(";")[1])
            sim = sim.split(";")[0]
...
@bot.on(mew_cmd(pattern="reverse"))
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await Meow.edit("Found Google Result. Processing results...")
        headers = {
...
More Info: Open this <a href="{the_location}">Link</a> in {ms} seconds""".format(
    ...
    await Meow.edit(OUTPUT_STR, parse_mode="HTML", link_preview=False)


@bot.on(mew_cmd(pattern="gps ?(.*)"))
