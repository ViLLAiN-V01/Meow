...
@bot.on(mew_cmd(pattern="anilist (.*)"))
...
@bot.on(mew_cmd(pattern="anime(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="anime(?: |$)(.*)", allow_sudo=True))
async def nope(hel_):
    Meow = hel_.pattern_match.group(1)
    if not Meow:
        if hel_.is_reply:
    ...
    troll = await bot.inline_query("AniFluidbot", f".anime {(deEmojify(Meow))}")
...
async def nope(hel_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".manga {(deEmojify(Meow))}")
...
@bot.on(mew_cmd(pattern="character(?: |$)(.*)"))
...
async def nope(hel_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".character {(deEmojify(Meow))}")
