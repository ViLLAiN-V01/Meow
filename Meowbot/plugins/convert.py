...
async def _(Meow):
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await Meow.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await Meow.client.send_file(
                event.chat_id,
...
@bot.on(mew_cmd(pattern="gif$"))
@bot.on(sudo_cmd(pattern="gif$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mewreply = await event.get_reply_message()
    if not mewreply or not mewreply.media or not mewreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if mewreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    mewevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with bot.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await bot.send_file(chat, mewreply.media)
            response = await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await mewevent.edit("`This file is not supported`")
            mewresponse = response if response.media else await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            mewfile = Path(await event.client.download_media(mewresponse, "./temp/"))
            mewgif = Path(await unzip(mewfile))
            mew = await bot.send_file(
                event.chat_id,
                mewgif,
            ...
            await bot(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=mew.media.document.id,
                        access_hash=mew.media.document.access_hash,
                        file_reference=mew.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
            await mewevent.delete()
            for files in (mewgif, mewfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await mewevent.edit("Unblock @tgstogifbot")
            return


@bot.on(mew_cmd(pattern="nfc ?(.*)"))
