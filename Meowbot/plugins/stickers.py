...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            Meow = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packname = f"{user.username}_Meowbot_{pack}"
        packnick = (
            f"{Meowbot} Vol.{pack}"
            if Meowbot
            else f"@{user.username}'s MeowBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            await Meow.edit("`Preparing a new pack....`")
            async with bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
@bot.on(mew_cmd(pattern=r"delst ?(.*)", outgoing=True))
...
@bot.on(mew_cmd(pattern=r"editst ?(.*)", outgoing=True))
    ...
    if Meow == "":
        ...
        async with bot.conversation(chat) as conv:
            try:
                ...
                await conv.get_response()
                await asyncio.sleep(2)
                await conv.send_message(f"{Meow}")
                response = await response
            except YouBlockedUserError:
                await event.reply("Please unblock @Stickers and try again")
                return
...
async def sticklet(event):
    ...
    FONT_FILE = await get_font_file(event.client, "@Meowfonts")
    ...
    image_stream.name = "Meowbot.webp"
...
@bot.on(mew_cmd(pattern="waifu(?: |$)(.*)", outgoing=True))
