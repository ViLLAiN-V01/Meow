...
async def set_not_afk(event):
    ...
    if "#" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        Meowbot = await event.client.send_message(
            event.chat_id,
        ...
        try:
            ...
            await bot.send_message(  # pylint:disable=E0602
                ...
                + f"Ask in {mew_grp} to get help!",
                reply_to=event.message.id,
                link_preview=False,
                silent=True,
            )
        await asyncio.sleep(5)
        await Meowbot.delete()
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        msg = await event.reply(message_to_reply, file=mewpic)
...
@bot.on(mew_cmd(pattern=r"afk (.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    mewop = await event.get_reply_message()
    global USER_AFK  # pylint:disable=E0602
    global afk_time  # pylint:disable=E0602
    global last_afk_message  # pylint:disable=E0602
    global afk_start
    ...
    mewpic = await event.client.download_media(mewop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {mewpic}"  # pylint:disable=E0602
        if reason:
            await bot.send_message(
                event.chat_id, f"**I'm going afk🚶** \n\n**Because :** {reason}", file=mewpic
            )
        else:
            await bot.send_message(
                event.chat_id, f"**I am Going afk!**🚶", file=mewpic)
...
@bot.on(mew_cmd(pattern=r"night ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_night
