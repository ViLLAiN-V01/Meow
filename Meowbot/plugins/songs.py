...
async def nope(mew):
    ...
    await eor(mew, f"Searching lyrics for  `{Meow}` ...")
    if not Meow:
        if mew.is_reply:
            (await mew.get_reply_message()).message
        else:
            await eod(mew, "Give song name to get lyrics...")
            return

    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(Meow))}")
...
@bot.on(mew_cmd(pattern="song(?: |$)(.*)", outgoing=True))
...
@bot.on(mew_cmd(pattern="vsong(?: |$)(.*)", outgoing=True))
