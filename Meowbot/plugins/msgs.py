...
async def all_groups_id(Meow):
    mewgroups = []
    async for dialog in Meow.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            mewgroups.append(entity.id)
    return mewgroups


@bot.on(mew_cmd(pattern="frwd$"))
...
@bot.on(mew_cmd(pattern=r"fpost (.*)"))
