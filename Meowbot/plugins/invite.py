...
async def get_users(event):
    ...
    if event.is_private:
        return await Meow.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await Meow.edit("**INVITING USERS !!**")
    async for user in event.client.iter_participants(mew.full_chat.id):
        try:
