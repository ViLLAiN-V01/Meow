...
@bot.on(mew_cmd(pattern="setgpic$"))
...
@bot.on(mew_cmd(pattern="promote(?: |$)(.*)"))
...
async def promote(promt):
    ...
    try:
        ...
        await mewevent.edit(f"**🔥 Promoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{promt.chat.title}`!! \n**Admin Tag :**  `{rank}`")
    except BadRequestError:
        await mewevent.edit(NO_PERM)
        return
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    mewevent = await eor(dmod, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        await mewevent.edit(NO_PERM)
        return
    await mewevent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{dmod.chat.title}`")
...
@bot.on(mew_cmd(pattern=r"mute ?(.*)"))
@bot.on(sudo_cmd(pattern=r"mute ?(.*)", allow_sudo=True))
async def muth(Meow):
    if Meow.is_private:
        await eor(Meow, "**Enough of your bullshit  !!**")
        await sleep(2)
        await Meow.get_reply_message()
        replied_user = await Meow.client(GetFullUserRequest(Meow.chat_id))
        if is_muted(Meow.chat_id, Meow.chat_id):
            return await Meow.edit(
                "Nigga is already muted here 🥴"
            )
        if Meow.chat_id == ForGo10God:
            return await eod(Meow, "Nashe me hai kya lawde 🥴")
        try:
            mute(Meow.chat_id, Meow.chat_id)
        except Exception as e:
            await eor(Meow, f"**Error **\n`{str(e)}`")
        else:
            await eor(Meow, "**Chup Reh Lawde 🥴\n`**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
    else:
        mewevent = await eor(Meow, "`Muting...`")
        input_str = Meow.pattern_match.group(1)
        chat = await Meow.get_chat()
        if Meow.reply_to_msg_id:
            userid = (await Meow.get_reply_message()).sender_id
            name = (await Meow.client.get_entity(userid)).first_name
        elif input_str:
            if input_str.isdigit():
                try:
                    ...
                    return await mewevent.edit(str(ve))
            else:
                userid = (await Meow.client.get_entity(input_str)).id
                name = (await Meow.client.get_entity(userid)).first_name
        else:
            return await eod(mewevent, "I Need a user to mute!!", 5)
        if userid == ForGo10God:
            return await eod(mewevent, "Nashe me hai kya lawde", 5)
        if str(userid) in DEVLIST:
            return await eod(mewevent, "**Error Muting God**", 7)
        try:
            await Meow.client.edit_permissions(
                chat.id,
                userid,
                until_date=None,
                send_messages=False,
...
@bot.on(mew_cmd(pattern=r"unmute ?(.*)"))
...
async def nomuth(evn):
    if evn.is_private:
        ...
        mewevent = await eor(evn, "`Unmuting...`")
        ...
        if evn.reply_to_msg_id:
            ...
            return await eod(mewevent, "I need a user to unmute!!", 3)
        try:
...
@bot.on(mew_cmd(pattern="ban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="ban(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def ban(bon):
    if bon.fwd_from:
        return
    mewevent = await eor(bon, "`Banning Nigga...`")
    ...
    if not user:
        return await mewevent.edit("`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await mewevent.edit("**Say again? Ban my creator??**")
    try:
    ...
    try:
        ...
        await mewevent.edit(f"**Banned  [{user.first_name}](tg://user?id={user.id})  in** `[{bon.chat.title}]` !!\n\nMessage Nuking : **False**")
        return
    if reason:
        await mewevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]` !!\n**Reason :** `{reason}`")
    else:
        await mewevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]`!!")
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    mewevent = await eor(unbon, "`Unbanning...`")
    ...
    try:
        ...
        await mewevent.edit(f"[{user.first_name}](tg://user?id={user.id}) **Is Now Unbanned in**  `{unbon.chat.title}` !!")
        ...
        await mewevent.edit("Invalid UserId!! Please Recheck it!!")


@bot.on(mew_cmd(pattern="pin($| (.*))"))
...
async def kick(usr):
    ...
    mewevent = await eor(usr, "`Kicking...`")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await mewevent.edit(NO_PERM + f"\n`{str(e)}`")
        return
    if reason:
        await mewevent.edit(
        ...
        await mewevent.edit(f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`")
...
@bot.on(mew_cmd(pattern=f"zombies ?(.*)"))
