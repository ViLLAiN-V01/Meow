...
@bot.on(mew_cmd(pattern=r"ocr (.*)", outgoing=True))
...
async def parse_ocr_space_api(event):
    ...
    try:
        ...
        await eod(Meow, "**Errors !!** \n`{}`\n**Report This to** {}\n\n`{}`".format(
                str(e), mew_grp, json.dumps(test_file, sort_keys=True, indent=4)
            )
        )
    else:
        await Meow.edit("Read Document in {} seconds. \n{}".format(
                ProcessingTimeInMilliseconds, ParsedText
            )
        )
    os.remove(downloaded_file_name)
    await Meow.edit(ParsedText)
